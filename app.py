...
# Configuração para salvar o banco de dados no servidor
base_dir = os.path.abspath(os.path.dirname(__file__))
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(base_dir, 'rpg.db')
...
def del_p(id):
    db.session.delete(Personagem.query.get(id))
    db.session.commit()
    return redirect(url_for('home'))

@app.route('/del_c/<int:id>')
def del_c(id):
    db.session.delete(Cronica.query.get(id))
    db.session.commit()
    return redirect(url_for('home'))

@app.route('/editar_p/<int:id>', methods=['POST'])
def editar_p(id):
    # Se estiver usando banco de dados (SQLAlchemy):
    p = Personagem.query.get(id)
    if p:
    ...
    # Se estiver usando listas (Memória):
